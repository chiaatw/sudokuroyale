...
impl Parse {
    /// Returns a new [ParsePacked] that ignores errors and wont perform any 
...
/// Parses puzzle strings into boards, optionally stopping on errors and/or
/// automatically solving naked and hidden singles
#[derive(Default)]
...
impl Parser for ParsePacked {
    /// Builds a new board using an input string to set some cells and returns
    /// it along with any actions and errors that arise
    /// 
    /// Use a digit (1 to 9) to set a cells value
...
/// Parses puzzle strings into boards with the exact solved cells and candidates from the grid format
#[derive(Default)]
...
impl Parser for ParseGrid {
    /// Builds a new board using an input string to set some cells and 
    /// returns it along with any actions and errors that arise
    fn parse(&self, input: &str) -> (Board, Effects, Option<(Cell, Known)>) {
        let mut board = Board::new();
...
/// Parses puzzle string into boards with the exact given/solved cells and candidates
#[derive(Default)]
